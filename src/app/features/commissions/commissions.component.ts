              ...
              <tbody>
                @if (loadingDetail()) {
                  @for (_ of [1,2,3]; track $index) {
                    <tr><td colspan="8"><div class="h-4 bg-slate-100 dark:bg-slate-700 rounded animate-pulse"></div></td></tr>
                  }
                } @else {
                  @for (r of commissions(); track r.id) {
                    <tr>
                      ...
                      <td><span class="badge-gray text-xs">{{ r.commissionType === 'PERCENTAGE' ? '%' : 'Fijo' }}</span></td>
                      <td class="text-sm">{{ r.commissionType === 'PERCENTAGE' ? (r.commissionRate + '%') : ('Bs. ' + r.commissionRate) }}</td>
                      <td class="font-semibold text-primary-600">Bs. {{ r.commissionAmount | number:'1.2-2' }}</td>
                      <td>
                      ...
                      <td>
                        @if (r.status === 'PENDING' && isAdmin()) {
                          <button (click)="openPayModal(r)" class="btn-success text-xs px-3 py-1.5 flex items-center gap-1">
                        ...
                        @if (r.status === 'PAID' && r.paidAmount != null) {
                          <span class="text-xs text-slate-400">Bs. {{ r.paidAmount | number:'1.2-2' }}</span>
                        }
                      </td>
                    </tr>
                  } @empty {
                    <tr><td colspan="8" class="text-center py-8 text-slate-400">Sin comisiones para este período</td></tr>
                  }
                }
              </tbody>
            </table>
          ...
          <div class="modal max-w-md animate-slide-up" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body space-y-4">
              ...
              <div class="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                  <span class="text-slate-500">Tipo comisión</span>
                  <span>{{ confirmPay()!.commissionType === 'PERCENTAGE' ? (confirmPay()!.commissionRate + '%') : ('Bs. ' + confirmPay()!.commissionRate) }}</span>
                </div>
                <div class="flex justify-between border-t border-slate-200 dark:border-slate-600 pt-2 mt-1">
                  <span class="text-slate-500 font-medium">Total comisión</span>
                  <span class="font-bold text-emerald-600 text-base">Bs. {{ confirmPay()!.commissionAmount | number:'1.2-2' }}</span>
                </div>
              </div>

              <!-- Opciones de pago -->
              <div>
                <label class="label mb-2">¿Cuánto se pagará?</label>
                <div class="grid grid-cols-3 gap-2">
                  <button type="button"
                    ...
                    100%
                    <div class="text-xs mt-0.5 opacity-80">Bs. {{ confirmPay()!.commissionAmount | number:'1.2-2' }}</div>
                  </button>
                  <button type="button"
                    (click)="setPayMode('50')"
                    ...
                    class="border-2 rounded-xl p-3 text-center transition-all font-semibold">
                    50%
                    <div class="text-xs mt-0.5 opacity-80">Bs. {{ (confirmPay()!.commissionAmount * 0.5) | number:'1.2-2' }}</div>
                  </button>
                  <button type="button"
              ...
              @if (payMode() === 'manual') {
                <div>
                  ...
                  <input type="number" [(ngModel)]="customPayAmount" min="0.01" [max]="confirmPay()!.commissionAmount"
                    step="0.01" class="input" placeholder="0.00"
                    (input)="cdr.markForCheck()">
                  <p class="text-xs text-slate-400 mt-1">Máximo: Bs. {{ confirmPay()!.commissionAmount | number:'1.2-2' }}</p>
                </div>
              }
...
export class CommissionsComponent implements OnInit {
  ...
  calcPaidAmount(): number {
    ...
    if (!rec) return 0;
    const full = Number(rec.commissionAmount);
    if (this.payMode() === '100') return Math.round(full * 100) / 100;
    if (this.payMode() === '50') return Math.round(full * 0.5 * 100) / 100;
    return Math.round((this.customPayAmount || 0) * 100) / 100;
  }

  canConfirmPay(): boolean {
    if (this.payMode() !== 'manual') return true;
    return this.customPayAmount > 0 && this.customPayAmount <= Number(this.confirmPay()?.commissionAmount || 0);
  }
  ...
  doMarkPaid() {
    ...
    this.api.patch(`/doctors/${this.selectedDoctorId}/commissions/${record.id}/mark-paid`, { paidAmount }).subscribe({
      next: () => {
        this.commissions.update(list => list.map(r => r.id === record.id ? { ...r, status: 'PAID', paidAmount } : r));
        this.loadSummary();
      },
      error: () => {},
    });
